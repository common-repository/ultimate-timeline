<?php
	...
	$all_posts = wp_count_posts( 'weblizar_timeline')->publish;	
    ...
    $wct_post_per_page  = esc_attr($attribute['post_per_page']);
    $wct_post_order     = $attribute['order'];
    $wct_post_per_page  = $wct_post_per_page ? $wct_post_per_page : $all_posts;
    ...
    if ( $wtl_loop->have_posts() ){
        while ( $wtl_loop->have_posts()) : $wtl_loop->the_post();
            ...
            $story_style        = 'div.custom_story_icon_'.esc_attr( $p_id ).'{ background-color : '.WeblizarTimelineHelper::get_story_color( $p_id ).'; color : white; color : '.WeblizarTimelineHelper::get_story_title_color( $p_id ).' }';
            $story_style        = 'div.custom_story_icon_'.esc_attr( $p_id ).'{ background-color : '.WeblizarTimelineHelper::get_story_color( $p_id ).'; color : white; color : '.WeblizarTimelineHelper::get_story_paragraph_color( $p_id ).' }';
            $story_style        = 'div.custom_story_icon_'.esc_attr( $p_id ).'{ background-color : '.WeblizarTimelineHelper::get_story_color( $p_id ).'; color : white; color : '.WeblizarTimelineHelper::get_story_date_color( $p_id ).' }';
            ...
            $html .= '<article class="timeline-entry '.esc_attr($even_odd).'" '.esc_attr($story_animation_css).'>
                        <div class="timeline-entry-inner">';
                            if ( $display_author_name ){
                                $author = ' - <small>'. esc_attr__( $author_name , 'ultimate-timeline' ) .'</small></small>';
                            }
...
<div class="container weblizar_timeline_container">
    <?php if( $timeline_title !== '' ): ?>
        <h4 class="text-center mt-0 mb-5 text-light text-uppercase">
            <?php echo esc_attr__( $timeline_title ,'ultimate-timeline' ) ?>
        </h4>
    <?php endif; ?>
    <div class="row">
