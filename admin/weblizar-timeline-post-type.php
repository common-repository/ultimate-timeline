...
class WeblizarTimelinePostType {
    ...
    public static function wct_submitbox_metabox() {
        if ((isset($_REQUEST['post']) && get_post_type($_REQUEST['post']) == 'weblizar_timeline') ||
            ...
            $html .= '<span class="custom_date_side_meta_box">' . esc_html__('*Please select story Date / Year from settings below the story content.', 'ultimate-timeline');
            $html .= ' <a href="#wct_story_date"><br/> - ' . esc_html__('Timeline Story Settings (Date/Year)', 'ultimate-timeline') . '</a>';
            $html .= '</span>';
            $html .= '</div>';
            echo $html;
        }
    ...
    public static function wct_submit_code_metabox() {
        if ((isset($_REQUEST['post']) && get_post_type($_REQUEST['post']) == 'weblizar_timeline') ||
            ...
            $html  = '<div class="misc-pub-section ctl-notice">';
            $html .= '<span class="">' . esc_attr__('Copy below shortcode in any Page to publish your Timeline', 'ultimate-timeline');
            $html .= ' <input type="text" class="form-control mt-2" value="[weblizar_timeline]">';
            $html .= '</span>';
            $html .= '</div>';
            echo $html;
        }
